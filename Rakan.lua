...
local menu = menu("Rakan", "Shadow Rakan")
    menu:header("name", "Shadow Rakan")
	...
	menu:menu("q", "Q Settings")
        ...
        menu.w:boolean("wc", "Use [Q] in combo", true)
...
local function AutoShield()
    if menu.e.ea:get() then
        for i=0, objManager.allies_n-1 do
          ...
          if player.pos:dist(obj.pos) >= 650 then return end
          if player:spellSlot(2).state ~= 0 then return end
...
print("Loaded ShadowAIO Shadow - v" .. Version)
